...
resource "azurerm_public_ip" "pip4" {
  name                = "${var.prefix}-${var.environment}-aks-pip4"
  location            = azurerm_resource_group.aks.location
...
resource "azurerm_public_ip" "pip6" {
  name                = "${var.prefix}-${var.environment}-aks-pip6"
  location            = azurerm_resource_group.aks.location
  resource_group_name = azurerm_kubernetes_cluster.aks.node_resource_group
  allocation_method   = "Static"
  zones               = ["1", "2", "3"]
...
resource "azurerm_public_ip_prefix" "prefix4" {
  name                = "${var.prefix}-${var.environment}-aks-prefix4"
  resource_group_name = azurerm_resource_group.aks.name
  location            = azurerm_resource_group.aks.location
  ip_version          = "IPv4"
  prefix_length       = "31"
...
resource "azurerm_public_ip_prefix" "prefix6" {
  name                = "${var.prefix}-${var.environment}-aks-prefix6"
  resource_group_name = azurerm_resource_group.aks.name
  location            = azurerm_resource_group.aks.location
  ip_version          = "IPv6"
  prefix_length       = "127"
