...
func main() {
	...
	if err = (&controller.ApiReconciler{
		...
		setupLog.Error(err, "Failed to create controller", "controller", "api")
		os.Exit(1)
	}
	if err = (&controller.ApiVersionReconciler{
		...
		setupLog.Error(err, "Failed to create controller", "controller", "apiversion")
		os.Exit(1)
	}
<<<<<<< HEAD
	// nolint:goconst
	if os.Getenv("ENABLE_WEBHOOKS") != "false" {
		if err := webhookv1alpha1.SetupBackendWebhookWithManager(mgr); err != nil {
			setupLog.Error(err, "Failed to create webhook", "webhook", "Backend")
			os.Exit(1)
		}
	}
=======
