...
var _ = Describe("Vault e2e", Ordered, func() {
	...
	BeforeAll(func() {
		...
		cmd = utils.Kubectl("delete", "vaults.keyvault.azure.com", expectedASOVaultName, "-n", sampleNamespace, "--ignore-not-found=true")
		_, _ = utils.Run(cmd)
		cmd = utils.Kubectl("delete", "roleassignments.authorization.azure.com", expectedRoleAssignName, "-n", sampleNamespace, "--ignore-not-found=true")
		_, _ = utils.Run(cmd)
	})
