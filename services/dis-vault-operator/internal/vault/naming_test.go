...
func TestDeterministicAzureVaultName(t *testing.T) {
	...
	if nameA != nameB {
		t.Fatalf("expected deterministic output, got %q and %q", nameA, nameB)
	}

	if len(nameA) == 0 || len(nameA) > 24 {
		t.Fatalf("expected AKV name length 1..24, got %d (%q)", len(nameA), nameA)
	}

	if matched := regexp.MustCompile(`^[a-z0-9-]+$`).MatchString(nameA); !matched {
		t.Fatalf("expected AKV-safe characters [a-z0-9-], got %q", nameA)
	}

	if !regexp.MustCompile(`.*-[a-f0-9]{6,8}$`).MatchString(nameA) {
		t.Fatalf("expected stable hash suffix, got %q", nameA)
	}
}
