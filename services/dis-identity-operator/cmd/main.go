...
func main() {
	...
	f.StringVar(&configFile, "config", "dis-identity-config.toml", "Path to toml config file.")
	opts := zap.Options{
		Development: true,
	}
	...
	mgr, err := ctrl.NewManager(ctrl.GetConfigOrDie(), ctrl.Options{
		...
		LeaderElectionID:       "30a6d3f5.dis.altinn.cloud",
	...
	if err := (&controller.ApplicationIdentityReconciler{
		...
		setupLog.Error(err, "Failed to create controller", "controller", "applicationidentity")
		os.Exit(1)
	}
