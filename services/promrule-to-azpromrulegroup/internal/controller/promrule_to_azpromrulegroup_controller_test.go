...
import (
	...
	"github.com/altinn/altinn-platform/services/promrule-to-azpromrulegroup/test/utils"
...
var _ = Describe("PrometheusRule Controller", func() {
	...
	Context("When reconciling a resource", func() {
		...
		BeforeEach(func() {
			...
			err = k8sClient.Get(ctx, typeNamespacedName, promRule)
		...
		It("should successfully reconcile a custom resource for PrometheusRule", func() {
			// Fake servers
			fakeDeploymentsServer := utils.NewFakeDeploymentsServer()
			fakePrometheusRuleGroupsServer := utils.NewFakeNewPrometheusRuleGroupsServer()
			// Clients
			...
			controllerReconciler := &PromRuleToAzPromRuleGroupReconciler{
				Client:                     k8sClient,
				...
				NodePath:                   "node",
				AzPromRulesConverterPath:   "/path/to/converter/tool",
			}
