...
func init() {
	...
	// k8s.IntOrString is no longer jsii-backed (see IntOrString.go) so it is
	// intentionally not registered with the jsii kernel here.
	...
	// k8s.Quantity is no longer jsii-backed (see Quantity.go) so it is
	// intentionally not registered with the jsii kernel here.
