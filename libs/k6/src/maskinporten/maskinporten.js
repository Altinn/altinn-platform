...
class MaskinportenAccessTokenGenerator {
    ...
    constructor(
        ...
        if (machineportenKid === undefined || machineportenClientId === undefined || encodedJwk === undefined) {
            throw Error("MaskinportenAccessTokenGenerator requires a maskinporten kid, client_id and and an encoded jwk")
        }
    ...
    #createJwtGrant(scopes) {
        ...
        const payload = {
            ...
            iat: now,
            exp: now + 0, // TODO allow config, by default it looks to be around 500s; 600 would mean 10 minute token. Double check this is actually true tho.
            jti: uuidv4(),
        };
        ...
        return function () {
            ...
            if (cache.has(key) && (cache.get(key).expiresAt - Date.now()) > 0) {
                ...
                try {
                    ...
                    throw new Error(`Failed to decode JWT payload for expiration: ${e.message}`);
                }

                if (expirationTimestamp <= Date.now()) {
                    throw new Error('Received token is already expired or has an invalid expiration date');
                }
                cache.set(key,
                    {
                        token: result,
