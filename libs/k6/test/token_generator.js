...
function testGetPersonalToken() {
    ...
    const token = tokenGenerator.getPersonalToken()
...
function testGetPersonalTokenBulk() {
    ...
    let tokens = tokenGenerator.getPersonalToken()
    try {
...
function testGetEnterpriseToken() {
    ...
    const tokenGenerator = new TokenGenerator(options)
    const token = tokenGenerator.getEnterpriseToken()
    if (!(token && typeof token === "string" && token.length > 0)) {
        throw new Error("Unexpected token value")
    }
}
...
function testGetEnterpriseTokenBulk() {
    ...
    let tokens = tokenGenerator.getEnterpriseToken()
    try {
        tokens = JSON.parse(tokens)
        tokens = new Map(Object.entries(tokens))
    } catch (e) {
