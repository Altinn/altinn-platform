...
var generateExamplesVersion = []string{"v1", "v2", "v3", "v4", "v5", "v6", "v7"}
...
func TestGetInfoFromFilePath(t *testing.T) {
	for _, tt := range splitPaths {
		...
		if actualDirName != tt.expectedDirName {
			t.Errorf("getTestScriptFileNameFromSplitString(%s): expected %s, actual %s", tt.path, tt.expectedDirName, actualDirName)
		}
		if actualFileName != tt.expectedFileName {
			t.Errorf("getTestScriptFileNameFromSplitString(%s): expected %s, actual %s", tt.path, tt.expectedFileName, actualFileName)
		}
		if actualTestScriptName != tt.expectedTestScriptName {
			t.Errorf("getTestScriptFileNameFromSplitString(%s): expected %s, actual %s", tt.path, tt.expectedTestScriptName, actualTestScriptName)
		}
		if actualDeployEnv != tt.expectedDeployEnv {
			t.Errorf("getTestScriptFileNameFromSplitString(%s): expected %s, actual %s", tt.path, tt.expectedDeployEnv, actualDeployEnv)
		}
	}
}
