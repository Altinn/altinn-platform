	...
	for _, td := range cf.TestDefinitions {
		...
		for _, c := range td.Contexts {
			if c.TestTypeDefinition.Enabled {
				...
				uniqName := fmt.Sprintf("%s-%s-%s", c.Environment, *c.TestRun.Name, randomString(5))
				...
				if utf8.RuneCountInString(*c.TestRun.Name) > 40 {
					log.Fatalf("Automatic generated name is too big: %s. The test_run name cannot be over 40 characters", uniqName)
				}
...
func (r K8sManifestGenerator) CallJsonnet(jp JsonnetParameters) {
	...
	cmd := exec.Command("jsonnet",
		...
		"--ext-str", fmt.Sprintf("is_browser_test=%t", jp.isBrowserTest),
